...
test("Outreach governance production route mounts SettingsWorkspace", async () => {
  const manifest = await read("../manifest.ts")
  const platformNavigation = await read("../../../app/platform-navigation.ts")
  const settingsPage = await read("../../../pages/settings-page.tsx")
  ...
  assert.doesNotMatch(settingsPage, /项目治理控制|人工确认发送/)
})
