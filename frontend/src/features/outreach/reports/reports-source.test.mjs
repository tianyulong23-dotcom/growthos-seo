...
test("Overview, Links, and Reports production routes use backend workspaces", async () => {
  ...
  const performanceWorkspace = await read(
    "../../../features/performance/performance-workspace.tsx"
  )
  const overview = await read("../../../pages/overview-page.tsx")

  assert.doesNotMatch(manifest, /\{ id: "reports"/)
  assert.match(platformNavigation, /\{ id: "reports", label: "指标与报告" \}/)
  assert.match(performanceWorkspace, /<ReportsWorkspace/)
  assert.match(performanceWorkspace, /<LinksWorkspace/)
  assert.doesNotMatch(performanceWorkspace, /ReportView|LinkMonitoringView/)
  assert.match(overview, /backlinksGetSummaryV1/)
  assert.match(overview, /backlinksProjectQueries\.fetch/)
  assert.doesNotMatch(
    overview,
    /有效推荐|今日下一步|最近活动|实时 Mock|Mock 数据/
  )
})
