...
const listColumns = `
  ...
  o.fulfillment_status "fulfillmentStatus",o.version,
  ...
  return Object.freeze({
    async listOpportunities(context, input) {
      ...
      const result = await client.query(`
        SELECT ${listColumns}
         ...
         WHERE (o.organization_id,o.workspace_id,o.website_project_id)=($1,$2,$3)
           ...
           AND ($5::text IS NULL OR o.management_status=$5)
           AND ($6::text IS NULL OR o.outcome_status=$6)
           AND ($7::text IS NULL OR o.fulfillment_status=$7)
           AND ($8::integer IS NULL OR o.join_sequence > $8
             OR (o.join_sequence=$8 AND o.id > $9::uuid))
         ORDER BY o.join_sequence ASC,o.id ASC
         LIMIT $10
      `, [context.tenant.organizationId, context.tenant.workspaceId,
        ...
        input.fulfillmentStatus ?? null, after?.[0] ?? null,
        after?.[1] ?? null, input.limit + 1]);
