...
import type {
  ...
  OpportunityCreation,
  OpportunityManagementPatch,
  OpportunityTransition,
  RecommendationFeedOpportunityCreation as FeedOpportunityCreation,
...
import {
  canTransitionOpportunityBusinessStage,
...
type CreateOpportunityFromInventoryCommand = Readonly<{
...
type CreateOpportunityFromFeedItemCommand = Readonly<{
  context: ResolvedProjectContext;
...
export type CreateOpportunityCommand =
  CreateOpportunityFromInventoryCommand | CreateOpportunityFromFeedItemCommand;
export type {
  RecommendationFeedOpportunityCreation,
  RecommendationFeedOpportunityRepository,
} from "../../db/repositories/opportunity.repository.js";
export type CreateOpportunityResult = (
  OpportunityCreation | FeedOpportunityCreation
) &
...
export function createOpportunityCommands(repository: OpportunityRepository) {
  return Object.freeze({
    async createFromRecommendation(
      ...
      if ("recommendationFeedItemId" in input) {
        const requestHash = digest({
          recommendationFeedItemId: input.recommendationFeedItemId,
        });
        const row = await repository.createFromRecommendationFeedItem({
          ...
          opportunityCreatedActionId: randomUUID(),
        });
        ...
        if (row.state === "not_found") {
          throw new BacklinkError({
            ...
            message: "Recommendation feed item was not found.",
          });
        }
      ...
      if (row.state === "version_conflict")
        throw conflict(
          "ExpectedVersion does not match the current recommendation.",
        );
    ...
    async transitionBusinessStage(
      ...
      const requestHash = digest({
        ...
        expectedVersion: input.expectedVersion,
        toBusinessStage: input.toBusinessStage,
        reason: input.reason,
      });
      ...
      if (row.state === "version_conflict")
        throw conflict(
          "ExpectedVersion does not match the current Opportunity.",
        );
      if (row.state === "invalid_transition")
        throw conflict(
          `Invalid Opportunity transition: ${row.currentStage} -> ${input.toBusinessStage}.`,
        );
      if (row.responseBody === undefined)
        throw conflict("The idempotent command is already in progress.");
      return { ...row.responseBody, replayed: row.state === "replay" };
    ...
    async patchManagement(
      ...
      if (row.state === "unchanged")
        throw conflict(
          `Opportunity management status is already ${input.managementStatus}.`,
        );
      if (row.responseBody === undefined)
        throw conflict("The idempotent command is already in progress.");
      return { ...row.responseBody, replayed: row.state === "replay" };
