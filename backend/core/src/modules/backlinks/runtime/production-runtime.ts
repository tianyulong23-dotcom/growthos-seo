...
} from "../application/backlinks.module.js";
import {
  createContactCommands,
} from "../application/commands/contacts.command.js";
import {
...
import type {
  createGmailConnectionCommands,
} from "../application/commands/gmail-connection.command.js";
import {
  createGmailConnectionCommands as createProductionGmailConnectionCommands,
} from "../application/commands/gmail-connection.command.js";
import {
  createOpportunityCommands,
} from "../application/commands/opportunities.command.js";
import {
...
} from "../application/commands/recommendations.command.js";
import {
...
import {
  createGmailConnectionQuery,
} from "../application/queries/gmail-connection.query.js";
import {
...
import type {
  SummaryQuery,
} from "../application/queries/summary.query.js";
import {
...
} from "../application/services/recommendation-refill-reservation.service.js";
import {
...
import type {
  BacklinksPrivateApiDependencies,
} from "../api/private-server.js";
import {
  verifiedPlatformContextForActor,
} from "../api/platform-request-context.js";
import {
...
} from "../db/repositories/oauth-attempt.repository.js";
import {
...
import {
  SafeFetchAdapter,
} from "../adapters/http/safe-fetch.adapter.js";
import {
...
import {
  createTemporalDraftGenerationScheduler,
} from "../workflows/draft-generation.starter.js";
import {
  createPlacementTemporalActivities,
} from "../workflows/placement.activities.js";
import {
...
type PlacementCandidateCommand =
  ReturnType<typeof createPlacementCandidateCommand>;
type PlacementReverifyCommand =
  ReturnType<typeof createPlacementReverifyCommand>;
...
type QueryFactory<T extends object> =
  (client: BacklinkTransactionClient) => T;
...
function localProductWorkerIdentity(): Readonly<{
  ...
  const websiteProjectId = process.env.LOCAL_PRODUCT_WEBSITE_PROJECT_ID;
  ...
  if (
    ...
    throw new Error("BACKLINKS_LOCAL_PRODUCT_WORKER_IDENTITY_MISSING");
  }
  return {
    scope: {
      organizationId: organizationId as string,
      workspaceId: workspaceId as string,
      websiteProjectId: websiteProjectId as string,
    },
...
function createScopedDraftGenerationRepository(
  ...
  ): Promise<T> => withBacklinkTenantTransaction(
    pool,
    tenantScopeFrom(input),
    (client) => operation(createDraftGenerationRepository(client)),
  );
  return Object.freeze({
    prepareEvidenceSnapshot: (input) =>
      scoped(
        input,
        (repository) => repository.prepareEvidenceSnapshot(input),
      ),
    createJob: (input) =>
    ...
    getJob: (input) =>
      scoped(input, (repository) => repository.getJob(input)),
    findLatestJob: (input) =>
...
function createScopedPlacementInitialValidationRepository(
  ...
  return {
    getCandidate: (input) => withBacklinkTenantTransaction(
      pool,
      tenantScopeFrom(input),
      (client) =>
        createPlacementInitialValidationRepository(client).getCandidate(input),
    ),
    record: (input) => withBacklinkTenantTransaction(
      pool,
      tenantScopeFrom(input),
      (client) =>
...
function createScopedPlacementMonitorRepository(
  ...
  return {
    prepare: (input) => withBacklinkTenantTransaction(
      pool,
      tenantScopeFrom(input),
      (client) => createPlacementMonitorRepository(client).prepare(input),
    ),
    scheduleRetry: (input) => withBacklinkTenantTransaction(
      pool,
      tenantScopeFrom(input),
      (client) => createPlacementMonitorRepository(client).scheduleRetry(input),
    ),
    complete: (input) => withBacklinkTenantTransaction(
      pool,
      tenantScopeFrom(input),
      (client) => createPlacementMonitorRepository(client).complete(input),
    ),
  };
}
...
function createProjectContextPort(pool: BacklinkTenantPool) {
  return {
    async resolve(input: Readonly<{
      ...
      if (platform.project.websiteProjectKey !== input.websiteProjectKey) {
        throw new BacklinkError({
  ...
  return {
    async create(input) {
      ...
      const rawResponse = row.responseBody as
        ...
        | undefined;
      const responseBody = rawResponse === undefined
        ? undefined
        : {
...
function createReportExportRepository(
  ...
  return {
    create: (record) => inScope(record, async (client) => {
      await client.query(
    ...
    get: (scope, exportId) => inScope(scope, async (client) => {
      const result = await client.query(
    ...
    claim: (scope, exportId) => inScope(scope, async (client) => {
      const result = await client.query(
    ...
    complete: (input) => inScope(input.scope, async (client) => {
      const result = await client.query(
        `UPDATE backlink_report_exports
    ...
    fail: (input) => inScope(input.scope, async (client) => {
      const result = await client.query(
        `UPDATE backlink_report_exports
...
function createSettingsGovernanceService(
  ...
  return {
    getView: (scope) => inScope(scope, async (client) => {
      ...
      return {
        ...
        retention: {
          ...
          rules: [...(retentionRow.rules as readonly {
            category: string;
            retainForDays: number;
          }[])],
          exceptions: [...(retentionRow.exceptions as readonly (
            | "legal_hold"
            | "audit_record"
            | "lifecycle_record"
            | "active_suppression"
    ...
    updateSettings: (input) => inScope(input.scope, async (client) => {
      const result = await client.query(
    ...
    updateKillSwitch: (input) => inScope(input.scope, async (client) => {
      const result = await client.query(
        `WITH current AS (
           SELECT version
             FROM backlink_kill_switch_versions
...
async function createApiDependencies(
  ...
  const replyMailContentReader = capabilities.gmailSyncEnabled
    ? createLocalProductReplyMailContentReader(resolve(
  ...
  const queries = Object.freeze({
    ...
    listMailMessages: scopedMethod(
      pool,
      replyMailFactory,
      "listMailMessages",
    ),
    getMailMessage: scopedMethod(
      pool,
      replyMailFactory,
      "getMailMessage",
    ),
    getMailThread: scopedMethod(pool, replyMailFactory, "getMailThread"),
    getSendIntent: scopedMethod(
      pool,
      sendIntentFactory,
      "getSendIntent",
    ),
  ...
  const contactFactory: QueryFactory<ContactCommands> =
    createContactCommands;
  const contactCommands: ContactCommands = Object.freeze({
    listCandidates: scopedMethod(
      pool,
      contactFactory,
      "listCandidates",
    ),
  ...
  const contactEnrichmentFactory: QueryFactory<ContactEnrichmentCommands> =
    (client) => createContactEnrichmentCommands(client, {
  ...
  const aiRuntime = capabilities.aiProviderEnabled
    ? createLocalProductAiRuntime({
        pool,
        secretStoreRoot: capabilities.secretStoreRoot
  ...
  if (!capabilities.googleOauthEnabled) {
    ...
    gmailConnectionCommands = createProductionGmailConnectionCommands({
      ...
      completion,
      disconnectWorkflow,
      redirectUri: capabilities.googleOauthRedirectUri
        ?? providerDisabled("Google OAuth Redirect URI"),
    });
  }
  ...
  const placementCandidateFactory:
    QueryFactory<PlacementCandidateCommand> = (client) =>
      createPlacementCandidateCommand({
  ...
  const placementCandidateCommand: PlacementCandidateCommand = Object.freeze({
    async execute(input) {
      const result = await withBacklinkTenantTransaction(
        ...
        tenantScopeFrom(input),
        (transaction) =>
          placementCandidateFactory(transaction).execute(input),
      );
  ...
  const placementReverifyFactory: QueryFactory<PlacementReverifyCommand> =
    (client) => createPlacementReverifyCommand(client);
  ...
  const reportExportWorkflow = createReportExportWorkflow({
    ...
    storage: {
      ...
      authorizeDownload: async () =>
        providerDisabled("Report Export storage"),
    },
    access: {
...
async function createWorkerRegistrations(
  ...
  const relay = createPlacementMonitoringRequestedOutboxRelay({
    repository: createOutboxRelayRepository(context.pool),
    consumer: createTemporalPlacementMonitoringInitializationConsumer(
      context.temporal.workflow,
      backlinksRuntimeContract.taskQueue,
      workerId,
    ),
  });
  const recommendationRefillRelay = capabilities.dataForSeoEnabled
    ? createRecommendationRefillOutboxRelay({
        repository: createOutboxRelayRepository(context.pool),
    ...
    : null;
  const contactEnrichmentRelay = createContactEnrichmentOutboxRelay({
    repository: createOutboxRelayRepository(context.pool),
    consumer: createTemporalContactEnrichmentConsumer(
      context.temporal.workflow,
      backlinksRuntimeContract.taskQueue,
    ),
  });
  const browserWorker = capabilities.browserProviderEnabled
    ? createSharedBrowserWorkerAdapter({
        endpoint: capabilities.browserWorkerEndpoint
    ...
    : Object.freeze({
        fetch: (request: Parameters<typeof placementSafeFetch.fetch>[0]) =>
          browserWorker.render({
            ...
            organizationId: workerIdentity.scope.organizationId,
  ...
  const placementActivities = createPlacementTemporalActivities({
    ...
    staticMonitorActivity:
      createPlacementStaticMonitorActivity(
        placementSafeFetch,
        placementBrowserFetch,
      ),
  });
  const placementMonitoringStarter =
    createTemporalPlacementMonitoringStarter(
      context.temporal.workflow,
      backlinksRuntimeContract.taskQueue,
    );
  ...
  const gmailSyncRuntime = capabilities.gmailSyncEnabled
    ...
    : null;
  const draftGenerationRepository =
    createScopedDraftGenerationRepository(pool);
  const aiRuntime = capabilities.aiProviderEnabled
    ? createLocalProductAiRuntime({
        pool,
        secretStoreRoot: capabilities.secretStoreRoot
    ...
    : null;
  const dataForSeoRuntime = capabilities.dataForSeoEnabled
    ? createLocalProductDataForSeoRuntime({
        pool,
        secretStoreRoot: capabilities.secretStoreRoot
    ...
    relayRun = relayRun.then(async () => {
      ...
      if (recommendationRefillRelay !== null) {
        const recommendationOutcome =
          await recommendationRefillRelay.runOnce({
            workerId,
            limit: 1,
            staleClaimBefore: new Date(Date.now() - 30_000),
          });
      ...
      if (workerIdentity !== null && Date.now() >= nextContactScanAt) {
        const created = await withBacklinkTenantTransaction(
          pool,
          workerIdentity.scope,
          (client) => ensureReadyContactEnrichmentJobs(client, {
            scope: workerIdentity.scope,
            actorId: workerIdentity.actorId,
            limit: 10,
            options: {
              maxPages: capabilities.contactEnrichmentMaxPages,
      ...
      if (
        workerIdentity !== null
        && Date.now() >= nextPlacementMonitorScanAt
      ) {
        const now = new Date();
        const due = await withBacklinkTenantTransaction(
          pool,
          workerIdentity.scope,
          (client) => createMonitoringScheduleRepository(client).listDue({
            ...workerIdentity.scope,
            dueAt: now,
            limit: 10,
          }),
  ...
  return Object.freeze({
    ...
    activities: Object.freeze({
      ...
      async backlinksLoadProjectAnalysisContextV1(input: Readonly<{
      ...
      async backlinksReserveRecommendationRefillV1(input: Readonly<{
        organizationId: string;
        workspaceId: string;
        websiteProjectId: string;
        jobId: string;
      ...
      async backlinksRecordRecommendationRefillFailureV1(input: Readonly<{
        organizationId: string;
        workspaceId: string;
        websiteProjectId: string;
        jobId: string;
        ...
        return withBacklinkTenantTransaction(pool, scope, async (client) => {
          const inserted = await client.query(
            ...
            [
              ...
              placementMonitoringConfiguration
      ...
      ...(gmailSendRuntime === null
        ? {}
        : {
            backlinksClaimGmailSendAttemptV1: (input: Parameters<
              typeof gmailSendRuntime.activity.claimAttempt
            >[0]) => gmailSendRuntime.activity.claimAttempt(input),
            backlinksDispatchGmailSendAttemptV1: (input: Parameters<
              typeof gmailSendRuntime.activity.dispatchAttempt
            >[0]) => gmailSendRuntime.activity.dispatchAttempt(input),
            backlinksSettleGmailSendAttemptV1: (input: Parameters<
              typeof gmailSendRuntime.activity.settleAttempt
            >[0]) => gmailSendRuntime.activity.settleAttempt(input),
          }),
      ...(gmailSyncRuntime === null
        ? {}
        : {
            backlinksRunGmailPollingSyncV1: (input: Parameters<
              typeof gmailSyncRuntime.run
            >[0]) => gmailSyncRuntime.run(input),
          }),
    }),
    backgroundServices: Object.freeze([Object.freeze({
      async start() {
