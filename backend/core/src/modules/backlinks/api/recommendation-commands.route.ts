...
export const requestRefillBodySchema = z.object({
  ...
  lowWatermark: z.number().int().nonnegative(), highWatermark: z.number().int().positive(),
  refillWindowKey: nonBlank.max(128),
}).strict().refine((body) => body.highWatermark > body.lowWatermark, {
  path: ["highWatermark"], message: "highWatermark must exceed lowWatermark." });
...
const refillResponse = z.object({ jobId: nonBlank, workflowId: nonBlank,
  status: z.literal("queued"), ...auditFields }).strict();
  ...
  api.post("/api/v1/projects/:websiteProjectKey/backlinks/recommendation-refill-jobs", {
    ...
    async (request, reply) => {
      ...
      const result = await options.commands.requestRefill({ context, requestId: request.id,
        idempotencyKey: request.headers["idempotency-key"], ...request.body });
