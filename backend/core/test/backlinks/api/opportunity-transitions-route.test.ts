...
const recommendationFeedItemId = "018f0000-0000-7000-8000-000000000082";
...
const context = {
  ...
  tenant: createTenantContext({
    ...
    workspaceId: "workspace-78",
  }),
  project: createProjectContext({
    websiteProjectId: "project-78",
    canonicalDomain: "example.com",
    locale: "en-US",
    countryCode: "US",
    profileVersionId: "profile-78",
    promotionTargetVersionId: "target-78",
  }),
};
...
describe("BL-AI-078/079 Opportunity command API", () => {
  it("enforces transition and management-patch guards", async () => {
    const commands = createOpportunityCommands({
      createFromRecommendation: async (input) =>
        input.expectedVersion === 7
          ...
          : {
              ...
              responseBody: {
                ...
                version: 1,
                lifecycleEventId: "life-create",
                auditEventId: "audit-create",
              },
            },
      createFromRecommendationFeedItem: async (input) => ({
        state: "completed",
        requestHash: input.requestHash,
        responseBody: {
          opportunityId,
          recommendationId,
          recommendationFeedItemId: input.recommendationFeedItemId,
          cycleId,
          joinSequence: 1,
          websiteProjectId: "project-78",
          targetSiteKey: "example.com",
          ...
          fulfillmentStatus: "NOT_EXPECTED",
          version: 1,
          lifecycleEventId: "life-create-v2",
      ...
      transitionBusinessStage: async (input) => {
        ...
        if (input.toBusinessStage === "AGREED")
          return {
        ...
        return {
          ...
          responseBody: {
            opportunityId,
            businessStage: input.toBusinessStage,
            managementStatus: "ACTIVE",
            outcomeStatus: "OPEN",
            fulfillmentStatus: "NOT_EXPECTED",
            version: 2,
            lifecycleEventId: "life-78",
            auditEventId: "audit-78",
          },
        };
      },
      patchManagement: async (input) =>
        input.expectedVersion === 7
          ? { state: "version_conflict", requestHash: input.requestHash }
          : {
              state: "completed",
              ...
              responseBody: {
                ...
                fulfillmentStatus: "NOT_EXPECTED",
                version: 2,
                lifecycleEventId: "life-79",
                auditEventId: "audit-79",
              },
            },
    });
    ...
    await app.ready();
    afterAll(() => app.close());
    const create = (project: string, expectedVersion: number, role?: string) =>
      app.inject({
        method: "POST",
        url: `/api/v1/projects/${project}/backlinks/opportunities`,
        headers: {
          "idempotency-key": `create-${expectedVersion}`,
    ...
    const createV2 = await app.inject({
      ...
      headers: { "idempotency-key": "create-v2" },
      payload: { recommendationFeedItemId },
    });
    expect(createV2.statusCode).toBe(201);
    expect(createV2.json()).toMatchObject({
      opportunityId,
      recommendationId,
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: "/api/v1/projects/project-key/backlinks/opportunities",
          headers: { "idempotency-key": "create-mixed" },
          payload: {
            recommendationId,
            recommendationFeedItemId,
            expectedVersion: 1,
    ...
    expect(
      (
        await app.inject({
          method: "POST",
          url: "/api/v1/projects/project-key/backlinks/opportunities",
          headers: { "idempotency-key": "create-missing" },
          payload: {},
        })
      ).statusCode,
    ).toBe(400);
    const post = (
      ...
      app.inject({
        ...
        headers: {
          "idempotency-key": `transition-${expectedVersion}-${toBusinessStage}`,
          ...(role === undefined ? {} : { "x-role": role }),
        },
        payload: {
          expectedVersion,
          toBusinessStage,
          reason: "Advance outreach.",
        },
      });
    expect(
      (await post("project-key", 1, "CONTACT_PREPARING")).json(),
    ).toMatchObject({
      opportunityId,
    ...
    expect((await post("project-key", 7, "CONTACT_PREPARING")).statusCode).toBe(
      409,
    );
    expect((await post("project-key", 1, "AGREED")).statusCode).toBe(409);
    expect(
      (await post("project-key", 1, "CONTACT_PREPARING", "viewer")).statusCode,
    ).toBe(403);
    expect((await post("foreign", 1, "CONTACT_PREPARING")).statusCode).toBe(
      403,
    );
    const patch = (project: string, expectedVersion: number, role?: string) =>
      app.inject({
        method: "PATCH",
        url: `/api/v1/projects/${project}/backlinks/opportunities/${opportunityId}/management`,
        headers: {
          "idempotency-key": `management-${expectedVersion}`,
          ...(role === undefined ? {} : { "x-role": role }),
        },
        payload: {
          expectedVersion,
    ...
    expect((await patch("project-key", 1)).json()).toMatchObject({
      ...
      version: 2,
      replayed: false,
      meta: { websiteProjectId: "project-78" },
    });
      ...
      (
        await app.inject({
          ...
          payload: {
            ...
            reason: "Do not overwrite business stage.",
          },
        })
      ).statusCode,
    ).toBe(400);
