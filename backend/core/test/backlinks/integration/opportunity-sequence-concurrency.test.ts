...
describe("BL-AI-077 project Opportunity counter", () => {
  ...
  beforeAll(async () => {
    ...
    try {
      ...
      const prospectIds = Array.from({ length: concurrency }, (_, index) =>
        id(1_000 + index),
      );
      ...
      const inventoryIds = Array.from({ length: concurrency }, (_, index) =>
        id(3_000 + index),
      );
      ...
      await client.query(
        `UPDATE backlink_recommendation_inventory inventory
  ...
  it("allocates 100 unique continuous and auditable sequences", async () => {
    const connectionUrl = new URL(harness.connectionString);
    // Avoid Docker Desktop's localhost dual-stack forwarding under 100 connections.
    ...
    try {
      ...
      expect(
        results.map((result) => result.joinSequence).sort((a, b) => a - b),
      ).toEqual(Array.from({ length: concurrency }, (_, index) => index + 1));
    } finally {
      await Promise.allSettled(clients.map((client) => client.end()));
    }

    const verifier = new PgClient({
    ...
    try {
      ...
      expect(
        (
          await verifier.query(`SELECT table_schema
        FROM information_schema.tables
        WHERE table_name='backlink_opportunity_project_counters'`)
        ).rows,
      ).toEqual([{ table_schema: "backlinks" }]);
      expect(
        (
          await verifier.query(`SELECT routine_schema
        FROM information_schema.routines
        WHERE routine_name='backlink_allocate_opportunity_join_sequence'`)
        ).rows,
      ).toEqual([{ routine_schema: "backlinks" }]);
      expect(
        (
          await verifier.query(
            `SELECT last_join_sequence,version,updated_by
