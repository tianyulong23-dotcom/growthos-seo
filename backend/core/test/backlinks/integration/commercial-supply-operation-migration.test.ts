...
describe("commercial supply operation migration", () => {
  ...
  beforeAll(async () => {
    ...
    await installBacklinksManifestAfterFoundation(client, "0071");
    ...
    `,
      [contextId, organizationId, workspaceId, websiteProjectId],
    );
    await client.query(
      `
    ...
    `,
      [jobId, organizationId, workspaceId, websiteProjectId],
    );
    await client.query(
      `
  ...
  it("replays without duplicating DDL or changing persisted facts", async () => {
    ...
    ).toEqual([{ name: "backlink_supply_operation_tenant_policy" }]);
    expect(
      (
        await client.query(`
    ...
    ).toEqual([{ rlsEnabled: true, rlsForced: true }]);
    expect(
      (
        await client.query(`
