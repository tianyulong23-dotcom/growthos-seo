...
const approvedDirectLicenses: Readonly<Record<string, string>> = {
  ...
  testcontainers: "MIT",
  tldts: "MIT",
  typescript: "Apache-2.0", "typescript-eslint": "MIT",
  validator: "MIT",
  vitest: "MIT", zod: "MIT",
};
const approvedSourceLicenses: Readonly<Record<string, string>> = {
  ...
  "OSS-MAIL-01": "MIT-0", "OSS-MAIL-02": "MIT-0",
  "OSS-MAIL-03": "MIT", "OSS-MAIL-04": "MIT",
  "OSS-RUNTIME-01": "MIT", "OSS-RUNTIME-02": "Artistic-2.0",
  "OSS-HTML-01": "MIT", "OSS-HTML-02": "MIT", "OSS-HTML-03": "MIT",
  "OSS-PLC-01": "MIT", "OSS-PLC-02": "MIT",
  "OSS-REC-01": "MIT",
  "OSS-SEO-02": "MIT",
  "OSS-SEO-03": "ISC",
  "OSS-TEST-03": "MIT",
  "OSS-TOOL-01": "Apache-2.0", "OSS-TOOL-02": "MIT",
...
export function analyzeThirdPartyLicenses(packageJson: unknown, packageLock: unknown, sourceManifest: unknown): { errors: string[]; notice: string } {
  const errors: string[] = [];
  if (!isObject(packageJson) || !isObject(packageLock) || !isObject(sourceManifest)) {
    return { errors: ["package, lockfile, and source manifest must be objects"], notice: "" };
  }
  if (!isObject(packageLock.packages)) {
    return { errors: ["package-lock.json packages must be an object"], notice: "" };
  }
  ...
  for (const [path, entry] of Object.entries(packageLock.packages)) {
    ...
    if (!isText(license)) {
      ...
      errors.push(`package-lock ${path} uses incompatible or unapproved license ${license}`);
    }
  ...
  for (const [field, scope] of [["dependencies", "production"], ["devDependencies", "development"]] as const) {
  ...
  if (!Array.isArray(sourceManifest.sources)) {
    ...
    for (const [index, source] of sourceManifest.sources.entries()) {
      ...
      if (expected === undefined) {
        errors.push(`source manifest ${source.id} has no approved license baseline`);
      } else if (source.license !== expected) {
        errors.push(`source manifest ${source.id} license must equal ${expected}`);
      }
      if (!compatibleLicenses.has(source.license)) {
        errors.push(`source manifest ${source.id} uses incompatible license ${source.license}`);
      }
      if (source.id.startsWith("OSS-RUNTIME-")) {
        const values = [source.project, source.packageVersion, source.repository];
        if (!values.every(isText)) {
          errors.push(`source manifest ${source.id} has incomplete runtime notice fields`);
        } else {
          runtimeRows.push(
      ...
      if (isObject(source.sourceFileHashes) && Object.keys(source.sourceFileHashes).length > 0) {
        const values = [source.id, source.project, source.auditedCommit, source.repository];
        if (!values.every(isText)) {
          errors.push(`source manifest ${source.id} has incomplete source notice fields`);
        } else {
          sourceRows.push(
  ...
  inventory.sort();
  const fingerprint = createHash("sha256").update(inventory.join("\n")).digest("hex");
  const notice = [
    "# Third-Party Notices", "",
...
export async function checkThirdPartyLicenses(writeNotice = false): Promise<void> {
  const base = new URL("../", import.meta.url);
  const path = (value: string) => fileURLToPath(new URL(value, base));
  const paths = {
    package: path("package.json"), lock: path("package-lock.json"),
  ...
  const [packageText, lockText, manifestText] = await Promise.all([
    readFile(paths.package, "utf8"), readFile(paths.lock, "utf8"),
    readFile(paths.manifest, "utf8"),
  ]);
  const result = analyzeThirdPartyLicenses(
    JSON.parse(packageText), JSON.parse(lockText), JSON.parse(manifestText),
  );
  if (result.errors.length > 0) {
    throw new Error(`Invalid third-party licenses:\n- ${result.errors.join("\n- ")}`);
  }
  if (writeNotice) {
    ...
    throw new Error("THIRD_PARTY_NOTICES.md is stale; run npm run licenses:check -- --write");
  }
  const count = result.notice.match(/Locked package entries: (\d+)/)?.[1] ?? "0";
...
if (entrypoint !== undefined && fileURLToPath(import.meta.url) === resolve(entrypoint)) {
  checkThirdPartyLicenses(process.argv.includes("--write")).catch((error: unknown) => {
