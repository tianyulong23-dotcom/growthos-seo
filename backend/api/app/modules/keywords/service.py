...
from app.modules.settings.models import AIProviderSetting, DataForSEOProviderSetting
...
class SQLAlchemyKeywordRepository:
    ...
    async def cost_summary(
        ...
        return KeywordCostSummaryResponse(
            ...
            ai_cost_complete=(
                ai_requests == 0 or ai_requests_with_reported_cost == ai_requests
            ),
        )
                ...
                (
                    await session.scalars(
                        ...
                        .where(
                            KeywordExternalRequest.organization_id == organization_id,
                            KeywordBuildRun.project_id == project_id,
                            KeywordExternalRequest.status.in_(
                                ("uncertain", "charged_failed")
                            ),
                        )
                        .order_by(
    ...
    async def cleanup_operational_data(self, *, retention_days: int) -> int:
        ...
        async with self.sessions() as session:
            ...
            compacted_sources = await session.execute(
                ...
                .values(
                    metadata_json=KeywordSource.metadata_json.op("-")("raw_payload")
                )
            )
    ...
    async def operational_health(
        ...
        *,
        ...
        async with self.sessions() as session:
            ...
            request_counts = (
                await session.execute(
                    ...
                    .where(
                        KeywordExternalRequest.organization_id == organization_id,
                        KeywordBuildRun.status.in_(("queued", "running", "waiting", "blocked")),
                    )
                )
            ).one()
        ...
        return KeywordOperationalHealth(
            ...
            active_runs=int(run_counts.active or 0),
            waiting_runs=int(run_counts.waiting or 0),
    ...
    async def requeue_orphaned_metric_workflow(
        ...
        *,
        ...
        async with self.sessions() as session:
            ...
            if next_replay_count >= METRIC_ORPHAN_REPLAY_LIMIT:
                ...
                if run is not None:
                    ...
                    latest_version = await session.scalar(
                        select(
                            func.coalesce(func.max(KeywordBuildRun.result_version), 0)
                        ).where(
            ...
            next_workflow_id = (
                f"keyword-metrics:{job.run_id}:{replay_attempt}:{uuid4()}"
            )
